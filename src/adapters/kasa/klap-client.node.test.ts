...
test('KLAP client trims handshake1 body to Content-Length when an extra byte is received', async () => {
	...
	const server = http.createServer(async (request, response) => {
		...
		if (url.pathname === '/app/handshake1') {
			...
			response.end(Buffer.concat([payload, Buffer.from('\n')]))
			return
		}
...
test('KLAP client trims handshake1 body when Content-Length is also inflated by one byte', async () => {
	const credentials = {
		username: 'kent@example.com',
		password: 'secret-password',
	}
	...
	const server = http.createServer(async (request, response) => {
		...
		if (url.pathname === '/app/handshake1') {
			...
			response.setHeader('Content-Length', String(payload.length + 1))
			response.end(Buffer.concat([payload, Buffer.from('\n')]))
			return
		}
