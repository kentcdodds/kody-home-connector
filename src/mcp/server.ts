...
import { createCourtAdapter } from '../adapters/court/index.ts'
import { createGlobalCacheAdapter } from '../adapters/global-cache/index.ts'
...
import { createAudiobookAdapter } from '../adapters/audiobook/index.ts'
import { type HomeConnectorConfig } from '../config.ts'
import { buildHomeConnectorRuntimeMetadata } from '../home-connector-metadata.ts'
import {
...
export type HomeConnectorToolRegistry = {
	...
	listHttp(): Array<HomeConnectorHttpToolDescriptor>
	call(
		name: string,
		args?: Record<string, unknown>,
		context?: HomeConnectorToolCallContext,
...
export function createHomeConnectorMcpServer(input: {
	...
	registerTool(
		{
			...
			description:
				'Read runtime metadata for this home MCP server, including APP_COMMIT_SHA, connector id, public MCP URL, Node version, uptime, and feature flags.',
			inputSchema: {},
			annotations: {
				readOnlyHint: true,
				idempotentHint: true,
		...
		async () => {
			...
			return structuredTextResult(
				`Home connector metadata for ${metadata.connectorId} (${metadata.appCommitSha ?? 'unknown commit'}).`,
				metadata,
			)
		},
	)
	...
	registerTool(
		{
			...
			description:
				'Get analog line-in details for a Sonos player that supports audio input. Also reports the HDMI/TV (ARC) URI for Amp/HT players.',
			...playerScopedSchema(),
			annotations: {
				readOnlyHint: true,
			},
	...
	registerTool(
		{
			...
			description:
				'Switch a Sonos Amp/home-theater player to its HDMI/TV (ARC/SPDIF) input. Use this for Sport Court Amp and other HT devices. Do not use sonos_select_audio_input — that is analog line-in.',
			...playerScopedSchema(),
		},
		async (args) => {
			const playerId =
			...
			return structuredTextResult('Selected the Sonos TV/HDMI input.', result)
		},
	)
	...
	registerKasaHomeConnectorTools({
		registerTool,
		kasa,
	})
	...
	registerCourtHomeConnectorTools({
		registerTool,
		court,
	})
			...
			return {
				...
				listHttp() {
					return [...tools.values()].map((entry) => ({
						...entry.descriptor,
						...(entry.sdkInputSchema
							? { sdkInputSchema: entry.sdkInputSchema }
							: {}),
						...(entry.sdkOutputSchema
							? { sdkOutputSchema: entry.sdkOutputSchema }
							: {}),
					}))
				},
